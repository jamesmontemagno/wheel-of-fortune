...
- **Bankrupt** appears once on the round-one wheel and never lands twice in a row. It clears only current round winnings and held prizes. **Lose a Turn** preserves them. Both pass play to the next person.
- Solve the whole puzzle to bank your round winnings, with a $1,000 minimum, plus any trip and mystery prizes you claimed. Only the solver banks money. Case, spacing, and punctuation do not matter; wrong solutions pass the turn.
...
- The bonus puzzle board stays covered while the champion spins for an envelope. The selected bonus puzzle has no more than 30% of its letter tiles in the given letters R, S, T, L, N, E; repeated letters count separately, while spaces and punctuation do not.
...
- The bonus round starts with R, S, T, L, N, E given. Pick three more consonants and one vowel within 60 seconds; if that clock runs out, the solve starts with whatever letters were picked. The given and chosen letters appear together above the board while picking, but the board tiles stay hidden until picks end. Then solve within 45 seconds with unlimited guesses. The clock continues if you switch apps or open help.
